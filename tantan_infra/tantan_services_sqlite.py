...
# 데이터베이스 경로 설정 (우선순위 순)
# 1. Docker 컨테이너 내부 마운트 경로 (/app/dnb_db 볼륨 - ro 마운트)
#    SQLite는 WAL/journal 파일을 써야 해서 ro 마운트에서 직접 열 수 없음
#    → /tmp에 복사 후 사용
if os.path.exists('/app/dnb_db/database.db'):
    _src = '/app/dnb_db/database.db'
    _tmp = '/tmp/tantan_db_cache.db'
    # 원본보다 캐시가 오래됐거나 없으면 복사
    if not os.path.exists(_tmp) or os.path.getmtime(_src) > os.path.getmtime(_tmp):
        shutil.copy2(_src, _tmp)
    DB_PATH = _tmp
# 2. Vultr 프로덕션 직접 경로 (비도커 환경)
elif os.path.exists('/var/www/dnbsir/database.db'):
...
def update_store(store_id, fields: dict):
    ...
    try:
        ...
        c.execute(f"UPDATE stores SET {set_clause} WHERE store_id = ?", values)
        conn.commit()
        conn.close()
        return True, "수정 완료"
    except Exception as e:
...
def get_dashboard_stats(tab=None):
    ...
    try:
        ...
        if tab in ['stores', 'citizens', 'farmers'] or not tab:
            ...
            if tab == 'stores':
                # citizen/farmer제외 전체 가맹점 (가맹점/택리기사/쿠리어 등)
