...
def get_store_virtual_number(store_id):
    if hasattr(db, 'get_store_virtual_number'):
        return db.get_store_virtual_number(store_id)
    return None
...
import db_sqlite as sqlite_fallback

def log_sms(store_id, phone, category, message, status, response=""):
    """SMS 로그 저장 Wrapper"""
    sqlite_fallback.log_sms(store_id, phone, category, message, status, response)

def get_sms_logs(store_id=None, limit=50):
    """SMS 로그 조회 Wrapper"""
    return sqlite_fallback.get_sms_logs(store_id, limit)
...
def get_all_ledger_records(record_type=None, limit=50):
    if hasattr(db, "get_all_ledger_records"):
        return db.get_all_ledger_records(record_type, limit)
    return pd.DataFrame()
...
def save_delivery_order(data):
    if hasattr(db, 'save_delivery_order'):
        return db.save_delivery_order(data)
    # fallback to SQLite fallback directly if somehow missing from proxy
    import db_sqlite
    return db_sqlite.save_delivery_order(data)

def get_delivery_order(order_id):
    if hasattr(db, 'get_delivery_order'):
        return db.get_delivery_order(order_id)
    import db_sqlite
    return db_sqlite.get_delivery_order(order_id)

def acquire_delivery_order_lock(order_id):
    if hasattr(db, 'acquire_delivery_order_lock'):
        return db.acquire_delivery_order_lock(order_id)
    import db_sqlite
    return db_sqlite.acquire_delivery_order_lock(order_id)

def update_delivery_order_status(order_id, status, waybill_number=None, error_message=None):
    if hasattr(db, 'update_delivery_order_status'):
        return db.update_delivery_order_status(order_id, status, waybill_number, error_message)
    import db_sqlite
    return db_sqlite.update_delivery_order_status(order_id, status, waybill_number, error_message)

def get_store_deliveries(store_id):
    return db.get_store_deliveries(store_id)

def get_today_deliveries(store_id):
    if hasattr(db, 'get_today_deliveries'):
        return db.get_today_deliveries(store_id)
    return pd.DataFrame()

def update_delivery_status(delivery_id, store_id, status):
    if hasattr(db, 'update_delivery_status'):
        return db.update_delivery_status(delivery_id, store_id, status)
    return False
...
def save_ai_call_log(store_id, customer_phone, customer_name, intent, summary, audio_url="", event_type=None, event_details=None):
    if hasattr(db, 'save_ai_call_log'):
        return db.save_ai_call_log(store_id, customer_phone, customer_name, intent, summary, audio_url, event_type, event_details)
    return False

def get_ai_call_logs(store_id, limit=50):
    if hasattr(db, 'get_ai_call_logs'):
        return db.get_ai_call_logs(store_id, limit)
    return pd.DataFrame()
...
def delete_store(store_id):
    if hasattr(db, 'delete_store'):
        return db.delete_store(store_id)
    return False

def log_security_event(store_id, customer_phone, event_type, payload):
    if hasattr(db, 'log_security_event'):
        return db.log_security_event(store_id, customer_phone, event_type, payload)
    return False

def get_security_logs_summary(hours=24):
    if hasattr(db, 'get_security_logs_summary'):
        return db.get_security_logs_summary(hours)
    import pandas as pd
    return pd.DataFrame()

def get_crm_customers_by_tag(store_id, tag):
    if hasattr(db, 'get_crm_customers_by_tag'):
