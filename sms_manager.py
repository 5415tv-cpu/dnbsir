...
async def send_highway_alimtalk(customer_phone: str, driver_phone: str, store_id: str = "SYSTEM", driver_name: str = ""):
    """
    콜백 알림톡 발송 (부재중/수신완료 → 고객에게 답장 문자)
    실패 시 SMS Failover 처리
    """
...
def _sync_send_highway(customer_phone: str, driver_phone: str, store_id: str, driver_name: str = ""):
    ...
    sender_phone = config_.get('sender_phone', '') or driver_phone

    # KST 시간
    tz_kst = datetime.timezone(datetime.timedelta(hours=9))
    now_kst = datetime.datetime.now(tz_kst)
    time_str = now_kst.strftime("%Y-%m-%d %H:%M:%S")
    visit_date = now_kst.strftime("%m/%d(%a) 오전 중")
    ...
    # 가게 정보로 이름 보완
    if not driver_name:
        store_info = db.get_store(store_id) or {}
        driver_name = store_info.get("owner_name") or store_info.get("name") or "동네비서"

    # ★ 콜백용 링크: 실제 존재하는 택배 접수 페이지
    callback_link = f"{base_url}/delivery/request?ref={customer_phone}"

    # 주문번호 생성 (알림톡 템플릿 변수용)
    import random, string
    order_id = "DNB" + ''.join(random.choices(string.digits, k=8))

    # Failover SMS 문구
    fallback_msg = (
        f"[동네비서] 전화 주셔서 감사합니다.\n"
        f"택배 접수: {callback_link}\n"
        f"문의: {sender_phone}"
    )

    try:
        from solapi import SolapiMessageService
        msg_service = SolapiMessageService(api_key, api_secret)
        ...
        response = msg_service.send(message_data)

        try:
            db.log_sms(store_id, customer_phone, "HIGHWAY_ALIMTALK", fallback_msg, "SUCCESS", f"OK (KST: {time_str})")
        except: pass
        print(f"[Message Highway] AlimTalk 성공: {customer_phone} / 링크: {callback_link}")

    except Exception as e:
        print(f"[Message Highway] AlimTalk 실패({str(e)}). SMS Failover 시도...")
        ...
        try:
            ...
            db.log_sms(store_id, customer_phone, "HIGHWAY_SMS", fallback_msg, status, f"Err: {str(e)[:80]}")
        except: pass
